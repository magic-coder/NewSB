...
import com.qmx.member.service.ConsumptionService;
...
@Api(value = "会员消费管理", tags = "会员消费管理", description = "会员消费管理")
public class ConsumptionController extends BaseController {
    ...
    public RestResponse<PageDto<ConsumptionDto>> findList(@RequestParam("access_token") String access_token,
        ...
        Page<Consumption> page = consumptionService.query(params);
        for (Consumption record : page.getRecords()) {
            dto = new ConsumptionDto();
            BeanUtils.copyProperties(record, dto);
            list.add(dto);
        }
        PageDto<ConsumptionDto> pageDto = new PageDto<>(page.getTotal(), page.getSize(), page.getCurrent(), list);
    ...
    public RestResponse<ConsumptionDto> createDto(@RequestParam("access_token") String access_token,
        ...
        try {
            ...
            Consumption consumption = consumptionService.findByLevelId(dto.getLevelId());
            if (consumption != null) {
                return RestResponse.fail("重复的消费规则");
            }
            ...
            model = consumptionService.saveAll(model, dto);
    ...
    public RestResponse<ConsumptionDto> updateDto(@RequestParam("access_token") String access_token,
        ...
        try {
            Assert.notNull(dto, "数据不能为空");
            Consumption model = consumptionService.find(dto.getId());
            if (model.getLevelId().equals(dto.getLevelId())) {
            } else {
                Consumption cc = consumptionService.findByLevelId(dto.getLevelId());
                if (cc != null) {
                    return RestResponse.fail("重复的消费规则");
                }
            }
            ...
            SysUserDto userDto = getCurrentUser(access_token);
            model.setUpdateBy(userDto.getId());
            model = consumptionService.updateAll(model,dto);
            return RestResponse.ok(model);
        } catch (Exception e) {
            return RestResponse.fail(e.getMessage());
        }
    ...
    public RestResponse<Boolean> deleteDto(@RequestParam("access_token") String access_token,
        ...
        try {
            consumptionService.del(id, getCurrentId(access_token));
    ...
    public RestResponse<ConsumptionDto> findById(@RequestParam("id") Long id) {
        Consumption model = consumptionService.find(id);
        ConsumptionDto consumptionDto = new ConsumptionDto();
        BeanUtils.copyProperties(model, consumptionDto);
        AssociatedDto dto;
        ...
        consumptionDto.setMap(productTypeListHashMap);
        return RestResponse.ok(consumptionDto);
    }
